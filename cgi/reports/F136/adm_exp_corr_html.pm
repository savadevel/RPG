...
package F136::RpgAdmExportCorrToHtml;
...
#*******************************************************************************
{
    ref($_[0]) && die "Error, class F136::RpgAdmExportCorrToHtml can't use in inheritance\n";        
    my ($class) = shift;
...
#*******************************************************************************
{
    ...
    $set->{LOG}->out(RpgLog::LOG_I, "user want see correction data in HTML");
    ...
    $set->{LOG}->out(RpgLog::LOG_I, "correction data in HTML format was sending");
    return TRUE;
    
_WAS_ERROR:
    $set->{LOG}->out(RpgLog::LOG_I, "error prepare correction data in HTML format");              
    return FALSE;
}
