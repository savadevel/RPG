...
package admin::RpgAdminLogExportToHtml;
...
#*******************************************************************************
{
    ref($_[0]) && die "Error, class admin::RpgAdminLogExportToHtml can't use in inheritance\n";        
    my ($class) = shift;
...
#*******************************************************************************
{
    ...
    my $exp_desc = admin::RpgAdminLogExport::EXPORT_DESC()->{$self->{TARGET}};    
    my $exporter = new RpgExportToHtml(%{$set}, TT => $self->{TT});
    my $body     = $self->{SRC_DATA}->get_data('GET_BODY', $exp_desc->{data_from} || 0);
    my $paging   = undef;
    
    # постраничный вывод данных
    if (defined($exp_desc->{PAGING}))
    {
        my $stat  = $self->{SRC_DATA}->get_obj_data('GET_BODY', $exp_desc->{PAGING}{data_from}); # общее число записей грузим из источника номер 2
        
        $paging                  = {};
        $paging->{rows_on_page}  = $set->{SETT}->get($set->{SESSION}->report(), $exp_desc->{PAGING}{rows});
        $paging->{current_page}  = $set->{CGI}->param($exp_desc->{PAGING}{page}) || 1;
        $paging->{rows_on_table} = $stat->rows();                
    }
    
    # печать тела (шаблона)
    if (FALSE == $exporter->export
        (
            TITLE    => (exists($exp_desc->{title}) ? $exp_desc->{title} : undef),
            TEMPLATE => $exp_desc->{html_template},
            PARAM    =>
            {            
                errors       => $self->errstr,
                alerts       => $self->alerts,
                ...
                table        =>
                {
                    description  => (exists($exp_desc->{title}) ? $exp_desc->{title} : undef),
                ...
                # словари
                dictionary   => {map{$_ => $self->{SRC_DATA}->get_data($_)} keys(%{$exp_desc->{DICTIONARIES}})}
            },
            TABLES   =>
            {
                GET_DATA => 
                {
                    TITLE  => (exists($exp_desc->{title}) ? $exp_desc->{title} : undef),
                    DATA   => $body,
                    FIELDS => {map{$_ => $exp_desc->{FIELDS}->{$_}} (@{$self->{FIELDS_MAIN}}, @{$self->{FIELDS_OPT}})}
                }
            }
        ))
    {
        # если при выводе шаблона были ошибки, то пишим их в лог
        $set->{LOG}->out(RpgLog::LOG_E, "Error, exporter returns: %s", $exporter->errstr());
        goto _WAS_ERROR;
    }
...
#*******************************************************************************
{
    my ($self)    = shift;
    my ($set)     = $self->{PARAM};
    my ($fields)  = admin::RpgAdminLogExport::CGI_DESC()->{$self->{TARGET}}{FIELDS};
    my ($ret)     =
            {
