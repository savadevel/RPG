...
// ***************************************************************** 
function Folder(target, folderDescription, hreference, strImgOpen, strImgClose) //constructor 
{ 
  //constant data 
  ...
  this.nodeImg = 0;
  this.isLastNode = 0;
  this.iconSrc = ICONPATH + strImgOpen;
  this.iconSrcClosed = ICONPATH + strImgClose;
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a class=mng_tree href='javascript:clickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a class=mng_tree>" 
  ...
  if (this.level>0) 
    if (this.isLastNode) //the last child in the children array 
      ...
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "iLine.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
  ...
  { 
    ...
    docW = docW + "<span style='margin-left:5'>" + this.desc + "</span>" + "</a>"
  } 
  else 
    docW = docW + "<span style='margin-left:5'>" + this.desc + "</span>"
  docW = docW + "</td>"
...
{ 
  ...
  if (this.getID()!=fRoot.getID() && PERSERVESTATE && !this.isOpen) //closing
  {
...
{ 
  ...
  { 
        if (USEFRAMES)
          docW = docW + "<a class=mng_tree href='" + this.hreference + "' TARGET=" + this.target + " "
        else
          docW = docW + "<a class=mng_tree href='" + this.hreference + "' TARGET=_top "
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
      srcStr = ICONPATH + "lLine.gif"
    else
      if (this.isOpen)
        srcStr = ICONPATH + "lMinus.gif"  
      else
        srcStr = ICONPATH + "lPlus.gif"  
  } 
  else 
  { 
    if (this.nChildren == 0)
      srcStr = ICONPATH + "tLine.gif"
    else
      if (this.isOpen)
        srcStr = ICONPATH + "tMinus.gif"
      else
        srcStr = ICONPATH + "tPlus.gif"
  }   
  return srcStr;
}
...
function Item(itemDescription, itemLink, target, strImg) // Constructor 
{ 
  // constant data 
...
{ 
  ...
  if (this.level>0) 
    ...
    { 
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "lLine.gif' width=16 height=22></td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "iLine.gif><img src='" + ICONPATH + "tLine.gif' width=16 height=22></td>"
    } 
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=top>"
  if (USEICONS)
      docW = docW + "<a class=mng_tree " + fullLink  + " id=\"itemIconLink"+this.id+"\">" + "<img id='itemIcon"+this.id+"' " + "src='"+this.iconSrc+"' border=0>" + "</a>"
  else
          if (this.prependHTML == "")
  ...
  if (USETEXTLINKS) 
    docW = docW + "<a class=mng_tree " + fullLink + " id=\"itemTextLink"+this.id+"\">" + "<span style='margin-left:5'>" + this.desc + "</span></a>"
  else 
    docW = docW + "<span style='margin-left:5'>" + this.desc + "</span>"
...
function leftSideHTML(leftSideCoded) {
        ...
        for (i=0; i<leftSideCoded.length; i++)
        {
                ...
                {
                        retStr = retStr + "<td valign=top background=" + ICONPATH + "iLine.gif><img src='" + ICONPATH + "iLine.gif' width=16 height=22></td>"
                }
...
{ 
  ...
  if (folderObj.id!=fRoot.id && PERSERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""
...
function gFld(target, description, hreference, strImgOpen, strImgClose) 
{ 
  folder = new Folder(target, description, hreference, strImgOpen, strImgClose);
  return folder;
} 
 
function gLnk(target, description, linkData, strImg) 
{ 
  var fullLink = "";
  var protocol = "";
  ...
  linkItem = new Item(description, protocol+linkData, target, strImg)
  return linkItem 
} 
...
function preLoadIcons() {
        var auxImg
        auxImg = new Image();
        auxImg.src = ICONPATH + "iLine.gif";
        auxImg.src = ICONPATH + "lMinus.gif";
        auxImg.src = ICONPATH + "tMinus.gif";
        auxImg.src = ICONPATH + "lPlus.gif";
        auxImg.src = ICONPATH + "tPlus.gif";
        auxImg.src = ICONPATH + "blank.gif";
        auxImg.src = ICONPATH + "lLine.gif";
        auxImg.src = ICONPATH + "tLine.gif";
        auxImg.src = ICONPATH + "parent_closed.gif";
        auxImg.src = ICONPATH + "parent_open.gif";
        auxImg.src = ICONPATH + "report.gif";
}

//Open some folders for initial layout, if necessary
function setInitialLayout() {
  if (browserVersion > 0 && !STARTALLOPEN)
    clickOnNodeObj(fRoot);
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != fRoot.getID())
...
function CookieBranding(name) {
  if (typeof fRoot.treeID != "undefined")
    return name+fRoot.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
{ 
  ...
  //fRoot (with the site's data) is created in an external .js
  fRoot.initialize(0, true, "") 
  if (supportsDeferral && !STARTALLOPEN)
          fRoot.renderOb(null) //delay construction of nodes
  else {
    renderAllTree(fRoot, null);

    if (PERSERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(fRoot)
